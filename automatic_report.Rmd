...
if ((is.numeric(beta)) && (length(beta) == 1)){
  ...
  # Heterogeneity hard-coded
  delta <- mean(1/beta - 1, na.rm = TRUE)
} else{
  # Heterogeneity as a formula
  beta_vector <- model.matrix(beta, data = data)
  delta <- mean(1/beta_vector - 1, na.rm = TRUE)
}
...
|---------|----------------|----------------|
| $\epsilon'\epsilon$ | `r sum((moments$moment_optimum$epsilon*moments$moment_optimum$weight)^2)` | `r sum((moments$moment_first_step$epsilon*moments$moment_first_step$weight)^2)` |
...
data_prediction_augm2 <- capitulation::life_cycle_model(
  ...
  Hreceived_var = "hr",
  return_last = FALSE,
  get_capital_income = TRUE, additional_vars = "tr_age_2015")
...
```{r}
p <- capitulation:::plot_lorenz(data_prediction_augm2)
p
```
